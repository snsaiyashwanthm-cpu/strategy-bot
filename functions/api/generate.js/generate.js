/**
 * This is our secure Cloudflare Worker backend (v-Final, Smart RAG).
 *
 * This version contains the "A++" no-compromise solution.
 * 1. It STORES all 10 exhaustive case studies (our "proprietary data").
 * 2. It runs a local "simpleSearch" to FIND the 2 most relevant case studies.
 * 3. It then sends a smaller, faster, and hyper-relevant prompt to Gemini.
 *
 * This fixes the timeout error AND makes the output *more* robust.
 */

// -----------------------------------------------------------------
// SECTION 1: OUR PROPRIETARY "EXHAUSTIVE DEPTH" RAG KNOWLEDGE BASE
// -----------------------------------------------------------------
...
// -----------------------------------------------------------------
// SECTION 2: THE "SMART RAG" SEARCH FUNCTION
// -----------------------------------------------------------------

/**
 * This is our fast, "local" search.
 * It finds the 2 most relevant case studies to send to the AI.
 */
function simpleSearch(industry, objective, context) {
    const query = `${industry} ${objective} ${context}`.toLowerCase();
    const keywords = query.split(/\s+/).filter(word => word.length > 3); // Get all unique keywords
    ...
    // Sort by score (highest first) and take the top 2
    const top2Docs = scores.sort((a, b) => b.score - a.score).slice(0, 2);
    
    // Return *only* the content, formatted for the prompt
    return top2Docs.map(doc => doc.content).join('\n\n---\n\n');
}


// -----------------------------------------------------------------
// SECTION 3: THE AI'S "BRAIN" (PROMPT & SCHEMA)
// -----------------------------------------------------------------
        ...
        "part2_blueprint": {
            ...
            items: {
                ...
                properties: {
                    ...
                    // "UNBEATABLE" FEATURE (LIVE DATA-VIZ)
                    "chart_data": {
                        type: "OBJECT",
                        nullable: true,
                        properties: {
                            "type": { type: "STRING" }, // 'bar', 'pie', 'line', 'bubble' (for 2x2)
                            ...
                            // Specific to 2x2 matrix
                            "x_axis_label": { type: "STRING", nullable: true },
                            "y_axis_label": { type: "STRING", nullable: true }
                        }
                    }
                }
            }
...
// -----------------------------------------------------------------
// SECTION 4: THE MAIN CLOUDFLARE WORKER
// -----------------------------------------------------------------
...
export async function onRequestPost(context) {
    try {
        ...
        const { industry, persona, objective, clientContext } = requestData;
        ...
        // 3. --- RUN THE "SMART RAG" SEARCH ---
        // This is our new, reliable "no-compromise" step.
        const relevantCaseStudies = simpleSearch(industry, objective, clientContext);

        // 4. This is the "A++" Senior Partner Meta-Prompt.
        // It's now LIGHTER and FASTER because it only includes the RELEVANT case studies.
        const finalPrompt = `
            # YOUR ROLE
            ...
            * **Persuasion (Smart RAG):** You MUST reference the 1-2 'RELEVANT CASE STUDIES' provided above as social proof in the 'key_content' of relevant slides.
            * **Rigor:** Do NOT invent facts. 'data_needed' should state what analysis is *needed*.
            * **"UNBEATABLE" FEATURE 4 (Live Data-Viz):** For *at least one* slide (like market sizing or a 2x2), you MUST populate the 'chart_data' object. For a 2x2, use type: 'bubble' and provide x/y labels. For other charts, use 'bar' or 'pie'.
            * **Format:** Adhere *perfectly* to the JSON schema for every slide.
        `;
        
        // 5. Make the *real* API call from the secure backend to Google
        ...
        // 6. Send the successful response back to the frontend (index.html)
        const data = await response.json();
        
        const responseText = data.candidates[0].content.parts[0].text;
        const parsedData = JSON.parse(responseText);
        
        return new Response(JSON.stringify({ payload: parsedData }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        });

    } catch (error) {
        // This will catch timeouts, API key errors, etc.
